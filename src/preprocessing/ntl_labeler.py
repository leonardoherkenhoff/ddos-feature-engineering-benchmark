...
Topological Labeler & Header Injector for NTLFlowLyzer

Injects missing headers into raw NTLFlowLyzer output and applies Ground Truth 

based strictly on the attacker's source IP (Topological rule).
...
INPUT_DIR = "./data/interim/NTL_RAW"

OUTPUT_DIR = "./data/processed/NTL"
...
# NTLFlowLyzer exports raw data without headers. This map binds data to feature names.
...
def process_file_auto(file_path):
    ...
    """Detects actual column count, assigns headers, and applies topological labeling."""
    ...
    try:
        ...
        # Detect physical column count to truncate the master header if needed
        ...
        # Process chunks to evaluate IP rules and drop dummy labels

        reader = pd.read_csv(file_path, header=None, chunksize=CHUNK_SIZE, low_memory=False)

        for chunk in reader:

            data = chunk.iloc[:, :-1] # Drop NTL dummy label
...
def main():
    ...
    print("=== NTLFlowLyzer Topological Labeling ===")

    files = glob.glob(os.path.join(INPUT_DIR, "**", "*.csv"), recursive=True)
    for f in files:
        if os.path.basename(f).startswith("monitor_"):
            continue
